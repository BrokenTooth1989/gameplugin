...
void IETSystemUtil::requestUrl(std::string requestType, std::string url, std::string data, const std::function<void(bool, std::string)> &func) {
    ...
    request->setResponseCallback([=](HttpClient *sender, HttpResponse *response) {
        ...
        std::string bufffff;
        bool isEncrypt = true;
        if (_xxAesSignLen>0&&buffer->size() > 16 + 32 + _xxAesSignLen) {
            for (int i = 0; isEncrypt && i < _xxAesSignLen && i < buffer->size(); ++i) {
                isEncrypt &= (buffer->at(i) == _xxAesSign[i]);
            }
            if (isEncrypt)
            {
                //尝试解密
                unsigned char* responseString = (unsigned char *) malloc(buffer->size());
                std::copy(buffer->begin(), buffer->end(), responseString);
                ssize_t decrpytBufferSize;
                responseString = aesDecrypt(responseString, buffer->size(), &decrpytBufferSize);
                bufffff=std::string((char*)responseString);//TODO:这样子会不会有问题？
                free(responseString);
            }
        }else{
            isEncrypt=false;
        ...
        if(!isEncrypt)
        {
            bufffff=std::string(buffer->begin(), buffer->end());
        }
