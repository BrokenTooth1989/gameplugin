...
void AES128_CBC_decrypt_buffer_nocopy(uint8_t* input, uint32_t length, const uint8_t* key, const uint8_t* iv)
{
    uintptr_t i;
    uint8_t remainders = length % KEYLEN; /* Remaining bytes in the last non-full block */
    ...
    if(remainders)
    {
        ...
        memset(input+remainders, 0, KEYLEN - remainders); /* add 0-padding */
        state = (state_t*)input;
        InvCipher();
    }
}
